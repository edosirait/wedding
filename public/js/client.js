...
const comment = (() => {
    ...
    const tambahKomentarKeDaftar = (komentar) => {
        const daftarUcapan = document.getElementById('daftar-ucapan');
        const elemenKomentar = document.createElement('div');
        elemenKomentar.classList.add('komentar-item', 'rounded-4', 'shadow', 'p-3', 'mb-2');

        const ikonKehadiran = komentar.hadir === true || komentar.hadir === '1' || komentar.hadir === 1
        ...
        <div class="card-body bg-light shadow p-3 m-0 rounded-4">
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                <p class="text-dark text-truncate m-0 p-0" style="font-size: 0.95rem;">
                    <strong class="me-1">${komentar.nama}</strong><i class="fa-solid ${ikonKehadiran}"></i>
                </p>
                <small class="text-dark m-0 p-0" style="font-size: 0.75rem;">${new Date(komentar.timestamp).toLocaleString()}</small>
            </div>
            <hr class="text-dark my-1">
            <p class="text-dark mt-0 mb-1 mx-0 p-0" style="white-space: pre-line">${komentar.komentar}</p>
        </div>`;

        daftarUcapan.prepend(elemenKomentar);
    };

    const kirim = async () => {
        const nama = document.getElementById('form-nama').value;
        const hadirValue = document.getElementById('form-kehadiran').value;
        const komentar = document.getElementById('form-pesan').value;

        // Konversi nilai hadir ke boolean
        const hadir = hadirValue === '1'; // Pastikan ini adalah boolean true atau false

        if (!nama || !komentar || hadirValue === '0') {
        ...
        try {
            const response = await fetch('/api/comment', {
                ...
                body: JSON.stringify({ nama, hadir, komentar }) // hadir sudah boolean
            });
            ...
            tambahKomentarKeDaftar({
                nama,
                hadir: hadirValue, // Kirim nilai asli '1' atau '2' ke fungsi
                komentar,
    ...
    const muatKomentar = async () => {
        try {
            const response = await fetch('/api/comment', { method: 'GET' });
            const result = await response.json(); // 'result' mungkin lebih deskriptif
            if (result.error) {
            ...
            if (Array.isArray(result.data)) {
                ...
                result.data.forEach(komentar => {
                    tambahKomentarKeDaftar(komentar);
                });
            } else {
                console.error('Data komentar bukan array', result.data);
            }
        } catch (error) {
